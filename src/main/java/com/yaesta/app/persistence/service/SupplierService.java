...
public class SupplierService implements Serializable {
	...
	public Supplier save(Supplier entity, List<SupplierDeliveryCalendar> deliveryCalendar,
			List<SupplierContact> contactList, List<SupplierContact> removeContactList) {
	...
	public List<Supplier> getSuppliers() {
		...
		/*
		 * for(Supplier s:found){ Hibernate.initialize(s.getCategory());
				...
				if (!contactContainer.getContactList().isEmpty()) {
					...
					for (ContactVO cvo : contactContainer.getContactList()) {
						...
						if (!phoneContainer.getPhoneList().isEmpty() && j < phoneContainer.getPhoneList().size()) {
							...
							/*
							 * if(phoneContainer.getPhoneList().get(j).getExt()!
							 * =null){
							 * sc.setExt(phoneContainer.getPhoneList().get(j).
							 * getExt().get(0)); }
							 */
							j++;
						}
					...
					if (!emailContainer.getEmailList().isEmpty()) {
						...
						for (EmailVO evo : emailContainer.getEmailList()) {
							...
							if (!phoneContainer.getPhoneList().isEmpty() && i < phoneContainer.getPhoneList().size()) {
								...
								/*
								 * if(phoneContainer.getPhoneList().get(i).
								 * getExt()!=null){
								 * sc.setExt(phoneContainer.getPhoneList().get(i
								 * ).getExt().get(0)); }
								 */
								i++;
							}
				...
				for (Supplier supplier : allSuppliers) {
					...
					if (addressList != null && !addressList.isEmpty()) {
						...
						for (Address address : addressList) {
							...
							// !address.getFirstname().equals(Constants.supplierFlag))
							...
							// !address.getLastname().equals(Constants.supplierFlag))
							...
							// address.getAlias()!=null &&
							// !address.getAlias().equals(Constants.supplierFlag))
					...
					if (tramacoSupplierList != null && !tramacoSupplierList.isEmpty()) {
						for (TramacoSupplier tramacoSupplier : tramacoSupplierList) {
							...
							if (tramacoSupplier.getProveedor().trim().toUpperCase()
									.equals(supplier.getName().trim().toUpperCase())) {
								tramacoSupplier.setSupplier(supplier);
								tramacoSupplierRepository.saveAndFlush(tramacoSupplier);

								List<TramacoZone> zones = tramacoZoneRepository.findByProvinciaAndCanton(
										tramacoSupplier.getProvincia(), tramacoSupplier.getGestorEntrega());
	...
	private void sendNewSellerMail(Supplier supplier, UserResponseContainer urc) {
		if (supplier.getIsNew()) {
			 ...
			 * mailSmtpSupplierContactsNames.split("%"); String[] contactsEmails
			 * = mailSmtpSupplierContacts.split("%");
			 * 
			 * for(int j=0;j<contactsNames.length;j++){ MailParticipant cc = new
			 * MailParticipant(); cc.setEmail(contactsEmails[j]);
			 * cc.setName(contactsNames[j]); ccList.add(cc); }
			 * 
			 * mailInfo.setReceivers(ccList);
			 */
			...
			String generalText = "Se informa de la creación en el sistema del proveedor " + supplier.getName()
					...
					+ "Usted podra ingresar al sistema de SELLERS de YAESTA con las credenciales usuario <b>"
					+ urc.getUsuario().getUsuario() + "</b> y clave <b>" + urc.getUsuario().getPassword()
					+ "</b><br></br>" + "Para ingresar utilice el siguiente url " + sellerCenterUrl;
	...
	@Transactional
	public String updateTCCCode() {

		String result="EXITO";
		List<Supplier> supplierList = this.getSuppliers();

		if (supplierList != null) {
			for (Supplier sup : supplierList) {
				if (sup.getPostalCode() != null) {
					try {
						TramacoZone tz = this.tramacoZoneRepository.findOne(new Long(sup.getPostalCode()));
						if (tz != null) {
							List<CoberturaTCC> cobTCCList = coberturaTCCRepository.findByProvinciaAndCantonAndParroquia(
									tz.getProvincia(), tz.getCanton(), tz.getParroquia());
							if (cobTCCList != null) {
								CoberturaTCC cTCC = cobTCCList.get(0);
								sup.setTccCode(cTCC.getCodigo());
								supplierRepository.save(sup);
							}
						}
					} catch (Exception e) {
						System.out.println("Error actualizando codigo TCC");
						result="ERROR "+sup.getName() + " " +e.getMessage();
						continue;
					}
				}
			}
